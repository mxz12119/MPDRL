...
class env(object):
	...
	def near_negative_sampling(self,ent,target_type,max_len=7,lowest_level=1):
		...
		# if len(negtive_paris)>0:
		# 	t=random.choice(negtive_paris)
...
def forscheme():
	...
	search_list = ['<Albert_Einstein>', '<Bill_Gates>', '<Donald_Trump>', '<Barack_Obama>', '<Isaac_Newton>']
	res = []
	for i in hu.triple:

		e1, e2, r = i[0], i[1], i[2]
		for j in search_list:
			if (e1 == j or e2 == j) and r == '<isCitizenOf>':
				res.append(i)
	print(res)
	task_relation = '<isCitizenOf>'
	hu.init_relation_query_state(task_relation)
	# j=[['<Albert_Einstein>', '<Germany>', '<isCitizenOf>'], ['<Donald_Trump>', '<Germany>', '<isCitizenOf>'], ['<Albert_Einstein>', '<Switzerland>', '<isCitizenOf>'], ['<Bill_Gates>', '<United_States>', '<isCitizenOf>'], ['<Albert_Einstein>', '<Weimar_Republic>', '<isCitizenOf>'], ['<Albert_Einstein>', '<Netherlands>', '<isCitizenOf>'], ['<Bill_Gates>', '<Germany>', '<isCitizenOf>'], ['<Bill_Gates>', '<Romania>', '<isCitizenOf>'], ['<Albert_Einstein>', '<Kingdom_of_Prussia>', '<isCitizenOf>'], ['<Barack_Obama>', '<Israel>', '<isCitizenOf>'], ['<Barack_Obama>', '<Scotland>', '<isCitizenOf>'], ['<Donald_Trump>', '<Scotland>', '<isCitizenOf>'], ['<Albert_Einstein>', '<Kingdom_of_Württemberg>', '<isCitizenOf>'], ['<Donald_Trump>', '<Jamaica>', '<isCitizenOf>'], ['<Bill_Gates>', '<United_Kingdom>', '<isCitizenOf>'], ['<Barack_Obama>', '<Kenya>', '<isCitizenOf>'], ['<Bill_Gates>', '<Scotland>', '<isCitizenOf>'], ['<Barack_Obama>', '<Wales>', '<isCitizenOf>'], ['<Albert_Einstein>', '<Russia>', '<isCitizenOf>'], ['<Donald_Trump>', '<Panama>', '<isCitizenOf>'], ['<Barack_Obama>', '<United_States>', '<isCitizenOf>'], ['<Albert_Einstein>', '<Austria-Hungary>', '<isCitizenOf>'], ['<Albert_Einstein>', '<India>', '<isCitizenOf>'], ['<Albert_Einstein>', '<United_States>', '<isCitizenOf>'], ['<Barack_Obama>', '<Germany>', '<isCitizenOf>'], ['<Albert_Einstein>', '<Austria>', '<isCitizenOf>'], ['<Donald_Trump>', '<United_States>', '<isCitizenOf>'], ['<Isaac_Newton>', '<United_Kingdom>', '<isCitizenOf>']]
	#j = ['<Isaac_Newton>', '<United_Kingdom>', '<isCitizenOf>']
	j=['<Barack_Obama>', '<United_States>', '<isCitizenOf>']
	#j=['<Albert_Einstein>', '<United_States>', '<isCitizenOf>']

	print('find:', j)
	e1, e2, r = tuple(j)
	print('Object type:', hu.ent2type[e1])
	print('Object type:', hu.ent2type[e2])
	res3 = hu.path_traverse_BFS(e1, e2, 6)
	print(res3)
def main_pathwalk():
	rootpath = './data/NELL/'
	task = 'concept:teamplaysagainstteam'
	rootpath = rootpath + task + '/'
	datapath = {'type2id': rootpath + 'type2id.json', 'relation2id': rootpath + 'relation2id.json' \
		, 'graph': rootpath + 'graph.pkl', 'ent2type': rootpath + 'ent2type.json' \
		, 'entity2id': rootpath + 'entity2id.json'}
	hu = env(datapath)
	hu.init_relation_query_state(task)
	hu.filter_query(4, 5000)
	pairs = hu.filter_query
	random.shuffle(pairs)
	path = []
	for i in pairs:
		e1, e2 = i[0], i[1]
		p = hu.path_traverse_BFS(e1, e2, max_len=3)
		path.append(p)
	res_path = []
	for pi in path:
		for pii in pi:
	...
	with open(rootpath + task + '.pathswalk', 'w') as fin:
		wstr = ['\t'.join(i) for i in res_path]
		wstr = '\n'.join(wstr)
		fin.write(wstr)
